...
ignorePublish: false
---
この記事は、[Pythonista3 Advent Calendar 2022](https://qiita.com/advent-calendar/2022/pythonista3) の12日目の記事です。
...
そこで`webbrowser` モジュールを使い、GUI 上で確認ができるようになり、ディレクトリ間の移動も容易になったらおかげで、該当ファイルの確認も、普段のPythonista3 操作と同様におこなえるようになりました。
...
- [outputlog.py | pome-ta/pysta-UISounds](https://github.com/pome-ta/pysta-UISounds/blob/master/outputlog.py)
  - 実行時のOS と、`/System/Library/Audio/UISounds/` 情報を出力
...
`webbrowser` モジュールでも不都合は無いのですが、`UISounds/` の一覧が見れたように`ui` モジュール等を使って何かしらできそうですね（`webbrowser.open` の1行っての寂しさがあります（趣味の場合のみ限定））。
...
作者のgist [File Picker.py](https://gist.github.com/omz/e3433ebba20c92b63111) にも、同様のコードがあります。
